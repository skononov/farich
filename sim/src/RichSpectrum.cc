...
int RichSpectrum::ReadFile(const char* datafile)
{
...
void RichSpectrum::ReduceToRange(double x1,double x2,double defval)
{
...
void RichSpectrum::Scale(double c)
{
...
double RichSpectrum::Evaluate(double x)
{
...
G4MaterialPropertyVector* RichSpectrum::GetMPV() const
{
	G4MaterialPropertyVector* mpv=new G4MaterialPropertyVector;

	data_const_iterator first=data.begin(), last=data.end();

	for( ; first!=last; first++ ) {
		G4double PhotMom=kPhotMomWaveConv/(*first).first*eV;
		mpv->AddElement(PhotMom,(*first).second);
	}
